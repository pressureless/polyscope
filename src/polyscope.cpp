...
void readPrefsFile() {
  ...
  // We never really care if something goes wrong while loading preferences, so
  // eat all exceptions
  catch (...) {
...
void init(std::string backend) {
  ...
  contextStack.push_back(
      ContextEntry{ImGui::GetCurrentContext(), nullptr, false});
...
void pushContext(std::function<void()> callbackFunction, bool drawDefaultUI) {
  ...
  ImGuiContext* newContext =
      ImGui::CreateContext(render::engine->getImGuiGlobalFontAtlas());
  ImGuiIO& oldIO = ImGui::GetIO(); // used to copy below, see note
  ImGui::SetCurrentContext(newContext);

  render::engine->setImGuiStyle();
  ImGui::GetIO() =
      oldIO; // Copy all of the old IO values to new. With ImGUI 1.76 (and some
             // previous versions), this was necessary to fix a bug where keys
             // like delete, etc would break in subcontexts. The problem was
             // that the key mappings (e.g. GLFW_KEY_BACKSPACE -->
             // ImGuiKey_Backspace) need to be populated in io.KeyMap, and these
             // entries would get lost on creating a new context.
  contextStack.push_back(
      ContextEntry{newContext, callbackFunction, drawDefaultUI});

  if (contextStack.size() > 50) {
    // Catch bugs with nested show()
    throw std::runtime_error(
        "Uh oh, polyscope::show() was recusively MANY times (depth > 50), this "
        "is probably a bug. Perhaps "
        "you are accidentally calling show every time polyscope::userCallback "
        "executes?");
  };
...
void processInputEvents() {
  ...
  if (!io.WantCaptureMouse && !widgetCapturedMouse) {
    ...
    bool dragRight =
        !dragLeft && ImGui::IsMouseDragging(
                         1); // left takes priority, so only one can be true
    if (dragLeft || dragRight) {

      glm::vec2 dragDelta{io.MouseDelta.x / view::windowWidth,
      ...
      if (isRotate) {
        glm::vec2 currPos{io.MousePos.x / view::windowWidth,
                          (view::windowHeight - io.MousePos.y) /
                              view::windowHeight};
    ...
    if (ImGui::IsMouseReleased(0)) {
      ...
      if (dragDistSinceLastRelease < dragIgnoreThreshold) {
        ...
        std::pair<Structure*, size_t> pickResult =
            pick::evaluatePickQuery(io.DisplayFramebufferScale.x * p.x,
...
void renderScene() {
  ...
  if (render::engine->getTransparencyMode() == TransparencyMode::Pretty) {
    ...
    // Clear the final buffer explicitly since we will gradually composite in to
    // it rather than just blitting directly as in normal rendering.
    ...
    render::engine->setDepthMode(); // we need depth to be enabled for the clear
...
void renderSceneToScreen() {
  ...
  if (options::debugDrawPickBuffer) {
    ...
    render::engine->pickFramebuffer->blitTo(
        render::engine->displayBuffer.get());
  } else {
...
void buildPolyscopeGui() {
  ...
  if (ImGui::IsItemHovered()) {
    ...
    ImGui::SetNextWindowPos(
        ImVec2(2 * imguiStackMargin + leftWindowsWidth, imguiStackMargin));
  ...
  if (ImGui::TreeNode("Debug")) {
    ...
    if (showDebugTextures) {
      render::engine->showTextureInImGuiWindow(
          "Scene", render::engine->sceneColor.get());
      render::engine->showTextureInImGuiWindow(
          "Scene Final", render::engine->sceneColorFinal.get());
    }
    ImGui::TreePop();
  }
...
void buildStructureGui() {
  ...
  ImGui::SetNextWindowPos(ImVec2(imguiStackMargin, lastWindowHeightPolyscope +
                                                       2 * imguiStackMargin));
  ImGui::SetNextWindowSize(
      ImVec2(leftWindowsWidth, view::windowHeight - lastWindowHeightPolyscope -
                                   3 * imguiStackMargin));
  ...
  for (auto catMapEntry : state::structures) {
    ...
    if (ImGui::CollapsingHeader(
            (catName + " (" + std::to_string(structureMap.size()) + ")")
                .c_str())) {
...
void buildUserGuiAndInvokeCallback() {
  ...
  if (state::userCallback) {
    ...
    if (options::openImGuiWindowForUserCallback) {
      ImGui::SetNextWindowPos(
          ImVec2(view::windowWidth - (rightWindowsWidth + imguiStackMargin),
                 imguiStackMargin));
...
void buildPickGui() {
  if (pick::haveSelection()) {
    ...
    ImGui::TextUnformatted(
        (selection.first->typeName() + ": " + selection.first->name).c_str());
    ImGui::Separator();
...
void draw(bool withUI) {
  ...
  render::engine->bindDisplay();
  render::engine->setBackgroundColor(
      {view::bgColor[0], view::bgColor[1], view::bgColor[2]});
  ...
  if (withUI) {
    if (contextStack.back().drawDefaultUI) {
      ...
      // Note: It is important to build the user GUI first, because it is likely
      // that callbacks there will modify polyscope data. If we do these
      // modifications happen later in the render cycle, they might invalidate
      // data which is necessary when ImGui::Render() happens below.
...
void mainLoopIteration() {
  ...
  if (options::maxFPS != -1) {
    ...
    microsecPerLoop = (95 * microsecPerLoop) /
...
void show(size_t forFrames) {
  ...
  if (!state::initialized) {
    throw std::logic_error(options::printPrefix +
                           "must initialize Polyscope with polyscope::init() "
...
Structure* getStructure(std::string type, std::string name) {
  ...
  if (name == "") {
    if (sMap.size() != 1) {
      error("Cannot use automatic structure get with empty name unless there "
  ...
  if (sMap.find(name) == sMap.end()) {
    error("No structure of type " + type + " with name " + name +
          " registered");
    return nullptr;
  }
  return sMap[name];
...
void removeStructure(std::string type, std::string name, bool errorIfAbsent) {
  ...
  if (sMap.find(name) == sMap.end()) {
    if (errorIfAbsent) {
      error("No structure of type " + type + " and name " + name +
            " registered");
    }
    return;
  }
    ...
    for (auto entry : typeMap.second) {
      ...
      if (entry.first == name) {
        if (targetStruct == nullptr) {
          ...
          error("Cannot use automatic structure remove with empty name unless "
                "there is exactly one structure of that "
                "type registered. Found two structures of different types with "
                "that name: " +
...
void processLazyProperties() {
  ...
  //
  // This function is a workaround which polls for changes to options settings,
  // and performs any necessary additional work.
  ...
  // ground plane
  if (lazy::groundPlaneEnabled != options::groundPlaneEnabled ||
    ...
    if (!options::groundPlaneEnabled) {
      // if the (depecated) groundPlaneEnabled = false, set mode to None, so we
      // only have one variable to check
  ...
  if (lazy::groundPlaneHeightFactor.asAbsolute() !=
          options::groundPlaneHeightFactor.asAbsolute() ||
      lazy::groundPlaneHeightFactor.isRelative() !=
          options::groundPlaneHeightFactor.isRelative()) {
...
void updateStructureExtents() {
  ...
  glm::vec3 minBbox =
      glm::vec3{1, 1, 1} * std::numeric_limits<float>::infinity();
  glm::vec3 maxBbox =
      -glm::vec3{1, 1, 1} * std::numeric_limits<float>::infinity();
